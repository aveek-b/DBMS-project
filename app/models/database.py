...
columns = [desc[0].lower() for desc in cursor.description] if
...
# Create output variables for OUT parameters
...
query = "SELECT * FROM vw_student_dashboard WHERE student_id =
:student_id"
...
year_of_study, semester, batch_year, date_of_birth, gender,

blood_group=None, permanent_address=None, city=None,

state=None, pincode=None, parent_name=None, parent_phone=None):
...
:course, :branch, :year_of_study, :semester, :batch_year,
:date_of_birth,
...
'user_id': user_id, 'roll_number': roll_number,

'registration_number': registration_number, 'course': course,
'branch': branch,

'year_of_study': year_of_study, 'semester': semester,
'batch_year': batch_year,

'date_of_birth': date_of_birth, 'gender': gender, 'blood_group':
blood_group,

'permanent_address': permanent_address, 'city': city, 'state':
state,

'pincode': pincode, 'parent_name': parent_name, 'parent_phone':
parent_phone,
...
query = "SELECT * FROM vw_room_availability WHERE available_beds >
0"
...
SELECT r.*, hb.block_name, r.capacity - r.current_occupancy AS
available_beds
...
SELECT lr.*, ua.first_name || ' ' || NVL(ua.last_name, '') AS
approved_by_name
...
def create_leave_request(student_id, leave_type, from_date, to_date,
reason,

destination=None, contact_during_leave=None, parent_contact=None):
...
reason, destination, contact_during_leave, parent_contact, status,
created_at
...
'student_id': student_id, 'leave_type': leave_type, 'from_date':
from_date,

'to_date': to_date, 'reason': reason, 'destination': destination,

'contact_during_leave': contact_during_leave, 'parent_contact':
parent_contact,
...
SELECT hr.*, s.first_name || ' ' || NVL(s.last_name, '') AS
staff_name
...
query += " ORDER BY DECODE(day_of_week, 'MONDAY', 1, 'TUESDAY', 2,
'WEDNESDAY', 3, 'THURSDAY', 4, 'FRIDAY', 5, 'SATURDAY', 6,
'SUNDAY', 7), DECODE(meal_type, 'BREAKFAST', 1, 'LUNCH', 2,
'SNACKS', 3, 'DINNER', 4)"
...
def get_active_announcements(target_audience='ALL', block_id=None,
limit=10):
...
SELECT a.*, u.first_name || ' ' || NVL(u.last_name, '') AS
created_by_name
...
return execute_function('func_calculate_compatibility', [student1_id,
student2_id])
...
results = [dict(zip(columns, row)) for row in result_cursor]
...
query = "SELECT * FROM vw_warden_dashboard WHERE warden_id =
:warden_id"
...
query += """ AND (LOWER(u.first_name) LIKE LOWER(:search)
...
query += f" ORDER BY s.roll_number OFFSET {offset} ROWS FETCH NEXT
{per_page} ROWS ONLY"
...
w.warden_id, u.first_name || ' ' || NVL(u.last_name, '') AS
warden_name,

(SELECT COUNT(*) FROM rooms r WHERE r.block_id = hb.block_id) AS
total_rooms,

(SELECT NVL(SUM(r.capacity), 0) FROM rooms r WHERE r.block_id =
hb.block_id) AS total_capacity,

(SELECT NVL(SUM(r.current_occupancy), 0) FROM rooms r WHERE r.block_id =
hb.block_id) AS current_occupancy
...
return execute_query("SELECT * FROM vw_hostel_occupancy_stats ORDER BY
block_name")
...
return execute_query("SELECT * FROM vw_monthly_analytics",
fetch='one')
