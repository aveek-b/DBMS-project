...
get_student_by_user_id, get_student_dashboard_data,
get_student_complaints,

get_student_fines, get_student_leave_requests,
get_student_housekeeping_requests,

get_mess_menu, get_user_notifications, get_active_announcements,

get_complaint_categories, get_room_occupants,
get_compatibility_questions,

get_student_compatibility_responses, get_best_roommate_matches,

create_leave_request, execute_procedure, execute_dml,
...
from ..utils.compatibility import calculate_detailed_compatibility,
...
roommates = [o for o in occupants if o['student_id'] !=
student['student_id']]
...
complaints_list = get_student_complaints(student['student_id'],
status_filter)
...
{'name': 'p_complaint_id', 'direction': 'OUT', 'type':
'NUMBER'},

{'name': 'p_complaint_num', 'direction': 'OUT', 'type':
'STRING'},
...
flash(f"Complaint {result.get('p_complaint_num')} registered
successfully!", 'success')

else:

flash(result.get('p_message', 'Failed to register complaint.'),
'danger')
...
fines_list = get_student_fines(student['student_id'], status_filter)
