...
get_warden_by_user_id, get_warden_dashboard_data, get_block_students,

get_block_complaints, get_block_leave_requests, get_all_rooms,

execute_procedure, get_complaint_statistics
...
recent_complaints =
get_block_complaints(warden['assigned_block_id'], 'PENDING')

pending_leaves =
get_block_leave_requests(warden['assigned_block_id'], 'PENDING')

return render_template('warden/dashboard.html',

warden=warden, dashboard=dashboard_data,

recent_complaints=recent_complaints[:5],

pending_leaves=pending_leaves[:5])

\@warden_bp.route('/leave/<int:leave_id>/process',
methods=['POST'])
