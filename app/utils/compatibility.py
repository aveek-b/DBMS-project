...
def calculate_detailed_compatibility(responses1, responses2):
    ...
    r1_dict = {r['question_id']: r['response_value'] for r in responses1}
    r2_dict = {r['question_id']: r['response_value'] for r in responses2}
    ...
    for r1 in responses1:
        ...
        if q_id in r2_dict:
            ...
            if r1_val == r2_val:
            ...
            elif category in ['CLEANLINESS', 'NOISE_TOLERANCE', 'PERSONALITY']:
                ...
                times = ['9 PM', '10 PM', '11 PM', '12 AM', '1 AM', 'After 1 AM']
                try:
            ...
            breakdown.append({
                ...
                'match_percentage': round(match_score / weight * 100) if weight > 0 else 0
            })

    overall_score = round(matched_weight / total_weight * 100, 2) if total_weight > 0 else 0
